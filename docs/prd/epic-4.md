...
5. فلترة حسب النوع (شراء/بيع/شراكة/ترشيح/ملاحظات)
6. فلترة حسب المبلغ (نطاق) باستخدام `.gte()` و`.lte()` (للطلبات المالية فقط)
...
4. عرض معلومات مختصرة لكل طلب (الرقم، النوع، المستثمر، المبلغ/الموضوع، الحالة، التاريخ)
