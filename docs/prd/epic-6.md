...
## Story 6.4: إرسال إشعارات البريد الفورية

**As a** نظام عمليات،  
**I want** إرسال تنبيهات بريدية فورية إلى فريق العمليات والإدارة،  
**so that** يتم تصعيد الأحداث الحساسة دون الاعتماد على SMS.

### Acceptance Criteria

1. إعادة استخدام Queue البريد (`notification_jobs`) لإرسال رسائل فورية عند الأحداث الحرجة (تقديم طلب، طلب معلومات إضافية، الموافقة/الرفض، بدء/إكمال التسوية).
2. إنشاء قوالب بريدية خاصة بفريق العمليات مع ملخص الطلب وروابط لوحة التحكم.
3. دعم قوائم مستلمين قابلة للتهيئة عبر البيئة (`ADMIN_NOTIFICATION_EMAILS`) مع لغة افتراضية قابلة للتخصيص.
4. تسجيل كل تنبيه في جدول `notifications` وربطه بالطلب المعني.
5. التعامل مع الأخطاء وإعادة المحاولة باستخدام نفس سياسة الـ backoff في Story 6.3.
6. تغطية وحدات الاختبار للسيناريوهات الرئيسية.
