...
### Admin Dashboard – KPIs (Story 7.6)

- **Entry Route:** يبقى `/admin/dashboard` (نفس الصفحة).
- **Data Layer:** `useAdminDashboardStats` أعيدت صياغتها لإرجاع حقل `kpis` يحوي أربع مجموعات بيانات (أوقات المعالجة، Pending Info، نجاح رفع المرفقات، فشل الإشعارات). كل مجموعة تتضمن القيم الأولية والتنبيه المنطقي.
- **UI Cards:** شبكة من أربع بطاقات جديدة تعرض القيم التالية:
  - متوسط/وسيط/النسبة المئوية 90 لوقت المعالجة بالساعات.
  - إجمالي طلبات Pending Info، عدد المتجاوز للـ SLA (24 ساعة)، ونسبة التأخر.
  - معدل نجاح رفع المرفقات (عدد الطلبات التي تضمنت مرفقات ÷ الطلبات المطلوب منها مرفقات).
  - معدل فشل الإشعارات خلال آخر 30 يومًا.
- **Alerts:** عند تجاوز العتبات يتم عرض كتلة تحذير (خلفية صفراء) تضم نقاطًا موجزة عن الحالات المتجاوزة، مع دعم RTL الكامل.
- **Localization:** مفاتيح اللغة الجديدة أضيفت إلى `frontend/src/locales/adminDashboard.ts` لكل من العربية والإنجليزية.
