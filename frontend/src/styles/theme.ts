...
export function toCssVariables({ palette: colors, spacing: space, radius: radii, shadow: shadows, typography: type }: Theme) {
  return {
    ...
    '--gradient-brand': `linear-gradient(135deg, ${colors.brandPrimary} 0%, ${colors.brandPrimaryStrong} 100%)`,
    '--gradient-surface': `linear-gradient(160deg, ${colors.backgroundSurface} 0%, ${colors.backgroundAlt} 100%)`,
