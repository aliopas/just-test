...
export function CompanyGoalsTable({
  ...
  const { draggedId, handleDragEnd } = useDragAndDropOrder(goals, onOrderChange);
  ...
  if (isError) {
    return (
      <div style={stateStyle}>
        <span>{isArabic ? 'حدث خطأ في التحميل' : 'Error loading goals'}</span>
    ...
    return (
      <div style={emptyStyle}>
        <h3 style={emptyTitleStyle}>
          {isArabic ? 'لا توجد أهداف' : 'No goals found'}
        </h3>
        <p style={emptySubtitleStyle}>
          {isArabic ? 'ابدأ بإضافة هدف جديد' : 'Start by adding a new goal'}
        </p>
      </div>
    );
  }
      ...
      >
        <thead>
          <tr style={{ background: palette.backgroundAlt }}>
            ...
            <th style={thStyle}>{isArabic ? 'تاريخ الهدف' : 'Target Date'}</th>
