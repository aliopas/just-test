...
export function CompanyResourcesTable({
  ...
  const { draggedId, handleDragStart, handleDragEnd } = useDragAndDropOrder(resources, onOrderChange);
  ...
  if (isError) {
    return (
      <div style={stateStyle}>
        <span>{isArabic ? 'حدث خطأ في التحميل' : 'Error loading resources'}</span>
    ...
    return (
      <div style={emptyStyle}>
        ...
        <p style={emptySubtitleStyle}>
          {isArabic ? 'ابدأ بإضافة مورد جديد' : 'Start by adding a new resource'}
        </p>
      </div>
    );
  }
